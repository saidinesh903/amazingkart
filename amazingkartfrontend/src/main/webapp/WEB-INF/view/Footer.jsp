<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<style type="text/css">

.navbar-custom {
    color: #FFFFFF;
    background-color: #304FFE;
}
</style>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>amazing kart Footer</title>
</head>
<body>
	
	<div class="navbar navbar-inverse navbar-inverse navbar-fixed-bottom" style="height:15px;">
		<div class="container-fluid">
			<div class="navbar-text pull-left">
				<p><marquee>&copy; Online shopping Store <b><i>amazingkart</i></b> </marquee></p>
			</div>
			<div class="navbar-text noStyle pull-right">
				<a href="https://www.facebook.com/isaac.devavaram" target="_blank">
					<i class="fa fa-facebook fa-lg"></i>
				</a>
				<a href="https://twitter.com" target="_blank">
					<i class="fa fa-twitter fa-lg"></i>
				</a>
				<a href="http://linkedin.com" target="_blank">
					<i class="fa fa-linkedin fa-lg"></i>
				</a>
				<a href="https://plus.google.com/105018105039014863203" target="_blank">
					<i class="fa fa-google-plus fa-lg"></i>
				</a>
				<a href="https://github.com/IsaacDV" target="_blank">
					<i class="fa fa-github fa-lg"></i>
				</a>
			</div>

		</div>
	</div>

</body>
</html>